...
module cv32e40p_dual_fetch_pair_buffer (
  ...
  assign in_ready_o     = out_slot_free;
    ...
    if (!rst_n) begin
      ...
      if (in_fire) begin
        if (!first_valid_q) begin
          // A compressed instruction is emitted immediately as a serialized
          // single; an uncompressed instruction waits for a possible partner.
          if (in_compressed_i) begin
            out_valid_q            <= 1'b1;
            out_inst1_q            <= in_instr_i;
            out_pc1_q              <= in_pc_i;
            out_inst1_compressed_q <= 1'b1;
            out_inst2_valid_q      <= 1'b0;
            first_valid_q          <= 1'b0;
          end else begin
            first_valid_q      <= 1'b1;
          ...
          // Discontinuity or compressed second instruction: emit the older
          // buffered instruction as a single. The incoming instruction becomes
          // the new first instruction unless it is compressed; compressed input
          // is held as first and will be serialized on the next available cycle.
          out_valid_q            <= 1'b1;
          out_inst1_q            <= first_instr_q;
          out_pc1_q              <= first_pc_q;
      ...
      end else if (out_slot_free && first_valid_q && first_compressed_q) begin
        // Serialize a compressed instruction that was retained after a
        // discontinuity.
        out_valid_q            <= 1'b1;
        out_inst1_q            <= first_instr_q;
        out_pc1_q              <= first_pc_q;
